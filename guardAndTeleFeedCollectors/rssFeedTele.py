...
try:
    df = pd.read_pickle('rssFeedTele.pickle')
    ...
    df = pd.DataFrame(columns = ['published', 'title'])
    

#feed = "https://tvn24.pl/najnowsze.xml" #sa tez najwazniejsze
#feed = "https://tvn24.pl/polska.xml"
#feed = "https://tvn24.pl/swiat.xml"
#feed = "https://www.telegraph.co.uk/rss"
#feed = "https://www.telegraph.co.uk/news/rss"
feed = "https://www.telegraph.co.uk/rss.xml"
...
dfNew = pd.DataFrame(rows_list, columns=['published', 'title'])
df = pd.concat([dfNew, df], ignore_index=True)
#print(df.shape)
#print(df.head())
#print(df.head())
df.to_pickle('rssFeedTele.pickle')
import os
